...
Deploy Produzione
Upload tutti i file su web server
Configurare HTTPS (richiesto per PWA)
Verificare manifest.json e service-worker.js paths
Testare installazione PWA
📱 Utilizzo
Primo Avvio
Carica dati ISTAT (opzionale): Impostazioni → Dataset ISTAT
Configura compensi: Impostazioni → Prestazioni e Compensi Standard
Aggiungi operatori: Operatori → + Nuovo Operatore
Aggiungi pazienti: Pazienti → + Nuovo Paziente
Registra primo accesso: Nuovo → Compila form
Installazione App
Clicca su "📱 Installa App" quando appare il prompt
Oppure: Menu browser → Installa Accessi Infermieri
L'app sarà disponibile come applicazione standalone
...
Fork del repository
Crea branch feature (git checkout -b feature/AmazingFeature)
